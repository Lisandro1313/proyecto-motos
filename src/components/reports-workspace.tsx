...
export function ReportsWorkspace() {
  const { data, totals, resetData, importData } = useAgencyStore();
  ...
  function handleImportSnapshot(event: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        ...
        setMaintenanceMessage("Datos importados correctamente.");
      } catch {
        setMaintenanceMessage("No se pudo importar el archivo seleccionado.");
      }
    };
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">
